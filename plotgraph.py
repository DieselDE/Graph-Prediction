...
if __name__ == "__main__":
    ...
    comb_data = pr.read_data("pr_comb.tsv")      # Hybrid predictions
    ...
    # Plot actual prices
    plt.plot(actual_data[:163], label='Actual BTC Price', color='black', linewidth=2, alpha=0.8)
    
    # Plot predictions
    plt.plot(comb_data[:163], label='Hybrid (Combined)', color='blue', linewidth=1.5, linestyle='--', alpha=0.7)
    plt.plot(tema_data[:163], label='TEMA Prediction', color='green', linewidth=1.5, linestyle='--', alpha=0.7)
    plt.plot(pattern_data[:163], label='Pattern Prediction', color='red', linewidth=1.5, linestyle='--', alpha=0.7)
